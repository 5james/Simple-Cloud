...
/*  Function protoypes  */
__declspec(dllexport) int makeKey(keyInstance *key, BYTE direction, int keyLen,
    char *keyMaterial);

__declspec(dllexport) int cipherInit(cipherInstance *cipher, BYTE mode, char *IV);

__declspec(dllexport) int blockEncrypt(cipherInstance *cipher, keyInstance *key, BYTE *input,
    int inputLen, BYTE *outBuffer);

__declspec(dllexport) int blockDecrypt(cipherInstance *cipher, keyInstance *key, BYTE *input,
    int inputLen, BYTE *outBuffer);

__declspec(dllexport) int serpent_convert_from_string(int len, char *str, unsigned int *val);

__declspec(dllexport) char *serpent_convert_to_string(int len, unsigned int val[8], char *str);